...
folders_training = list(open('/data/estudiantes/kalau/MTFL/training_listaimagenes.txt'))
folders_testing = list(open('/data/estudiantes/kalau/MTFL/testing_listaimagenes.txt'))
...
for image in path_file_training:
    ...
    Pface_training.append(cv2.resize(cv2.cvtColor(cv2.imread(image), cv2.COLOR_BGR2GRAY), (60, 60), interpolation=cv2.INTER_AREA))
...
for ye in range(0, 4151):
    for yi in range(len(Ytraining[ye, :])):
        Ytraining[ye, yi] = (60.0 * Ytraining[ye, yi]) / 250.0

for ye in range(4151, 7650):
    for yi in range(len(Ytraining[ye, :])):
        Ytraining[ye, yi] = (60.0 * Ytraining[ye, yi]) / 400.0

for ye in range(7650, 10000):
    Ytraining[ye, 0::2] = (60.0 * Ytraining[ye, 0::2]) / 160.0
    Ytraining[ye, 1::2] = (60.0 * Ytraining[ye, 1::2]) / 216.0
...
for image in path_file_testing:
    # Reajustamos el tamaño de la imagen que es la permitida por la red neuronal en este caso (96,96
    Pface_testing.append(cv2.resize(cv2.cvtColor(cv2.imread(image), cv2.COLOR_BGR2GRAY), (60, 60), interpolation=cv2.INTER_AREA))
...
for ye in range(len(Ytesting)):
    for yi in range(len(Ytesting[ye, :])):
        Ytesting[ye, yi] = (60.0 * Ytesting[ye, yi]) / 250.0

#Implementación de la topología de la red neuronal convolucional
model = Sequential()
#Primer capa, 20 neuronas, el tamaño del filtro es (5,5), el tipo de activación y el tamaño de entrada
model.add(Conv2D(20, (5, 5), padding='same', activation='tanh', input_shape=(Spic_training, Spic_training, 1)))
model.add(MaxPool2D(pool_size=(2, 2), strides=(1, 1)))
model.add(Conv2D(48, (3, 3), padding='same', activation='tanh'))
model.add(MaxPool2D(pool_size=(2, 2), strides=(1, 1)))
model.add(Conv2D(64, (3, 3), padding='same', activation='tanh'))
model.add(MaxPool2D(pool_size=(2, 2), strides=(1, 1)))
model.add(Conv2D(80, (3, 3), padding='same', activation='tanh'))
model.add(Flatten())
...
model.fit(Xtraining, Ytraining, batch_size=128, epochs=1000, validation_data=(Xtesting, Ytesting), verbose=1)
#Guardar el modelo
